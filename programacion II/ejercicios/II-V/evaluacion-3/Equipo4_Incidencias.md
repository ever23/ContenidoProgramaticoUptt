# Evaluación 3: Sistema de Reporte de Incidencias

**Equipo 4**
**Dominio:** Soporte técnico y mantenimiento de infraestructura.

## 🎯 Objetivo
Desarrollar un sistema web asíncrono tipo CRUD para gestionar (reportar, visualizar, editar y eliminar) los fallos en equipos o en la infraestructura (ej. aire acondicionado, laboratorios), utilizando un enfoque de Arquitectura de 3 Capas, Programación Orientada a Objetos y Modularización.
...
1. **Campos del Formulario:**
   - Título del Problema (Texto)
   - Ubicación (Texto o Select, ej. "Laboratorio 2")
   - Nivel de Urgencia (Select: Baja, Media, Alta)
   - Descripción Detallada (Textarea)

2. **Validaciones Estrictas (Frontend y Backend):**
   - La **Descripción Detallada** debe tener un mínimo de 15 caracteres (para evitar reportes inútiles tipo "se dañó").
   - La **Ubicación** es obligatoria.
   - El **Nivel de Urgencia** debe tener un valor válido seleccionado.

3. **Persistencia de Datos:**
   - El archivo donde se guardará la información debe llamarse `incidencias.json`.

4. **Sección de Estadísticas Básicas:**
   - La interfaz debe incluir un panel que muestre siempre actualizado: **El total de incidencias** reportadas históricamente y **cuántas de ellas son de urgencia Alta**.

5. **Funcionalidad de Búsqueda:**
   - Añadir una barra de búsqueda que permita buscar o filtrar incidencias por **Ubicación** o **Nivel de Urgencia**.
...
- **Frontend:** Usar ES6 Modules (`<script type="module">`). Separar la lógica en distintos archivos (Ej. `Incidencia.js`, `UI.js`, `app.js`).
...
- **Frontend:** Instanciar el objeto antes de enviarlo (ej. `const nuevaInc = new Incidencia(...)`). Crear una clase gestora para la interfaz (ej. `class UI`) con métodos como `capturarFormulario()`, `mostrarError()`, y `renderizarLista()`.
