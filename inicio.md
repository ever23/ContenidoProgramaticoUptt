# 👨‍🏫 Repositorio Oficial de Cátedras
## Universidad Politécnica Territorial de Trujillo (UPTT) 1.0.0

Bienvenido al portal oficial de contenidos programáticos y manuales de estudio. Este repositorio ha sido diseñado bajo los estándares modernos de la industria para centralizar toda la información teórica y de laboratorio de las materias de ingeniería.

### 📚 Rutas de Aprendizaje Disponibles

*   **[Algorítmica y Programación](Algorítmica%20y%20Programación/documentacion/I-I/Lectura_Catedra_Evaluacion_1.md)** (Trayecto I)
*   **[Programación II](programacion%20II/documentacion/II-V/Lectura_Catedra_Evaluacion_1.md)** (Trayecto II)
*   **[Redes](Redes/documentacion/II-V/Lectura_Catedra_Evaluacion_1.md)** (Trayecto II)

### 📋 Documentación y Planificación
*   **Algorítmica:** [Contenido Programático](Algorítmica%20y%20Programación/Propuesta_Contenido_Programatico_2026.md)
*   **Programación II:** [Contenido Programático](programacion%20II/Propuesta_Contenido_Programatico_2026.md)
*   **Redes:** [Contenido Programático](Redes/Contenido_Programatico.md) 

---

> **💡 Tips para estudiantes:**
> * Utiliza el **menú lateral izquierdo** para navegar estructuradamente entre los diferentes trimestres y lecturas de cada materia. 
> * Si estás buscando un concepto específico (por ejemplo, "JSON" o "Bucle For"), puedes utilizar la **barra de búsqueda** ubicada en la parte superior izquierda.
> * Todos los bloques de código JavaScript tienen un **botón de copiado** en la esquina superior derecha para que puedas probarlos en tu editor.

---
*Profesor: Enyerber Franco - Ingeniero de Sistemas*
